...
class Converter(object):
    ...
    def get_entries(self, docname):
        uri_base = '/feeds/default/private/full'
        query = '?title=%s&title-exact=false&max-results=5' % docname
        feed = self._client.GetAllResources(uri=uri_base + query)
        if not feed:
    ...
    def convert(self, docname, dirname, author_map):
        ...
        if os.path.exists(dirname):
            ...
            if 'revision:' in o:
                ...
                starting_revision = int(rev_split[-1].split(' ')[-1])
        else:
            os.mkdir(dirname)
            os.chdir(dirname)
            os.system('git init')
        ...
        for rev in revision_feed.entry:
            ...
            os.system('pandoc -f html -t rst -o %s.rst %s.html' % (docname,
                                                                   docname))
            if os.path.exists('images'):
                os.system('git add %s.rst images/*.png' % docname)
            else:
                os.system('git add %s.rst' % docname)
            git_commit = 'git commit --date=%s --author="%s <%s>" -m ' \
                         '"revision:%d"'
            format_commit = git_commit % (date, name, email, revision_number)
            os.system(format_commit)
